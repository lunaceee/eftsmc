...
var escapeQuote=function(str){if(typeof(str)=="string"){str=str.replace(new RegExp("\\'","ig"),str)
}return str
};
...
return b
}}}catch(e){if(console&&typeof(console.warn)=="function"){console.warn("Error in Operator resolution",e,toEval)
}}return false
}}
}();
...
var d=function(){if(!a.areStoresLoaded){return
}var f=CQ_Analytics.StoreRegistry.getStores();
for(var g in f){var h=f[g];
if(h&&h.getName&&!h.initialized===void (0)){return
}}window.clearInterval(e);
a.fireEvent("storesinitialize");
a.areStoresInitialized=true
};
this.initTimer=window.setTimeout(function(){e=window.setInterval(d,100)
...
!function(){var b=this.require||this.amdShim&&this.amdShim.require;
if(b==null||this.CQ_Analytics==null){return
}var a="clientstate";
b(["shared/util/state","shared/util/state/adapters/onChange","shared/util/state/adapters/form","shared/util/state/adapters/order","shared/util/state/adapters/persistent","shared/lib/RSVP"],function(j,g,e,c,f,k){var d;
function h(l){return k.resolve().then(function(){return j.retrieve(l)
}).then(function(n){var p=n;
function o(r,s){var s=s?s+"":"";
var q=d.data[r];
if(q===s){return
}d.setProperty(r,s)
}function m(){for(var q in p){if(!p.hasOwnProperty(q)){continue
}o(l+"_"+q,p[q])
}}m();
g.on("change",function(q,r){var s=q.split(".")[1];
if(s==null){return
}o(l+"_"+s,r)
})
})
}k.all([new k.Promise(function(n,m){var l=CQ_Analytics.CCM.getStores();
if(l[a]==null){CQ_Analytics.CCM.addListener("storeregister",function(p,o){if(o==null||o.STORENAME==null){return
}if(o.STORENAME.toLowerCase()==a){n(o)
}})
}else{n(l[a])
}}),j.adapters.add(g),j.adapters.add(e),j.adapters.add(c),j.adapters.add(f)]).then(function(l){d=l[0]
}).then(function(l){return k.all([h("form"),h("persistent")])
})
})
}();
!function(){var j=window;
if(j.CQ_Analytics==null||j.CQ_Analytics.clientStateStore!=null){return
}var d="clientstate";
var f="generatedThumbnail";
var e={};
var a;
var k=false;
j.et=j.et||{};
j.et.state=j.et.state||[];
a={user_market:function(g){return g.user.market
},user_culture_code:function(g){return g.user.culture_code
},user_ef_id:function(g){return g.user.ef_id
},user_language:function(g){return(g.user.culture_code||"").substring(0,2)
},user_language_code:function(g){return g.user.language_code
},tracking_events:function(g){return(g.tracking||{}).events||[]
},page_environment:function(g){return g.page.environment
},page_market:function(g){return g.page.market
},page_language:function(g){return g.page.language
},page_culture_code:function(g){return g.page.culture_code
},page_application:function(g){return g.page.application
},page_area:function(g){return g.page.area
},page_cobrand:function(g){return g.page.partner_code||g.page.partner
},page_source:function(g){return(g.page.partner!=null&&g.page.partner_code=="none")?g.page.partner:g.page.partner_code
},page_etag:function(g){return g.page.etag
},page_canonical_path:function(g){return g.page.canonical_path
},page_root_relative_path:function(g){return g.page.root_path
},session_source:function(g){return g.session.partner_code||g.session.partner
},session_etag:function(g){return g.session.etag
},order_offer_id:function(g){return g.order.offer_id||""
}};
for(var l in a){if(!a.hasOwnProperty(l)){continue
}e[l]=null
}function b(g){j.et.state.get("user","session","page","tracking","order",function(o,r,q,s,n){var p={user:o,session:r,page:q,tracking:s,order:n||{}};
g&&g(p)
})
}var c={isApplicable:function(g){return true
},afterPut:function(o,g,n){b(function(q){var r;
for(var p in a){if(!a.hasOwnProperty(p)){continue
}r=a[p](q);
if(r!=e[p]&&CQ_Analytics.clientStateStore!=null){CQ_Analytics.clientStateStore.setProperty(p,r);
e[p]=r
}}})
}};
function m(){var g=this;
g.init()
}var h=new CQ_Analytics.SessionStore();
h.STORENAME=d;
h.STOREKEY=d.toUpperCase();
h.init=function(){var n=this;
n.data=e;
if(k){return
}k=true;
n.addListener("update",function g(o,p){if(p==f&&n.getProperty(f,true)!=false){e[f]=n.getProperty(f,true);
n.removeListener("update",g)
}});
j.et.state.push(null,function(){b(function(p){j.et.state.addAdapters(c);
for(var o in a){if(!a.hasOwnProperty(o)){continue
}e[o]=a[o](p)
}n.loadInitProperties(e,true);
n.initialized=true;
n.fireEvent("initialize",n)
})
})
};
m.prototype=h;
CQ_Analytics.clientStateStore=new m();
CQ_Analytics.CCM.addListener("configloaded",function(){CQ_Analytics.CCM.register(CQ_Analytics.clientStateStore)
},null)
}();
if(!CQ_Analytics.Silverpop){CQ_Analytics.Silverpop=new CQ_Analytics.SessionStore();
CQ_Analytics.Silverpop.STORENAME="silverpop";
CQ_Analytics.Silverpop.STOREKEY="SILVERPOP";
CQ_Analytics.Silverpop.data={}
}if(CQ_Analytics.ClientContextMgr){CQ_Analytics.ClientContextMgr.register(CQ_Analytics.Silverpop)
};